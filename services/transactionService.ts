...
export const createTransaction = async (
    ...
    try {
        const { id, type, walletId, goalId, amount, image, category, uid, description, date } = transactionData;
        const dateTransaction = new Date(date!).toISOString().slice(0, 19).replace("T", " ");
        ...
        if (type === "goal") {
            await updateGoalForNewTransaction(db, uid!, goalId!, Number(amount));
        }
        ...
        await db.runAsync(
            `INSERT INTO transactions (id, walletId, amount, type, image, category, date, description, uid) 
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [transcationId, walletId, amount, type, transactionData.image, category, dateTransaction, description, uid]
        );
...
const updateGoalForNewTransaction = async (
    ...
    try {
        const existingGoal = await db.getFirstAsync<GoalType>(
            `SELECT collected FROM goals WHERE id = ? AND uid = ?`,
            [goalId, uid]
        );
        ...
        await db.runAsync(
            `UPDATE goals SET collected = ? WHERE id = ? AND uid = ?`,
            [updatedCollected, goalId, uid]
        );
